...
/* codes: 台股代號陣列（如 ['2330','2382']）；symbols: 美股Ticker陣列（如 ['NVDA','MSFT']）
   逐一呼叫避免瞬間高併發觸發來源端流量限制，回傳 { code: {price,change,changePercent} } */
async function fetchQuotesBatch(codes, symbols) {
  const out = {};
  for (const code of codes) {
    try { out[code] = await fetchTwStockQuote(code); }
    catch (err) { console.error(`台股報價抓取失敗 ${code}`, err); out[code] = { code, price: null, change: null, changePercent: null }; }
  }
  for (const symbol of symbols) {
    try { out[symbol] = await fetchUsStockQuote(symbol); }
    catch (err) { console.error(`美股報價抓取失敗 ${symbol}`, err); out[symbol] = { code: symbol, price: null, change: null, changePercent: null }; }
  }
  return out;
}
