...
async function fetchDividendRate(fundId) {
  ...
  if (src.type === 'moneydj') {
    ...
    for (const url of urls) {
      ...
      if (parsed) {
        result = { fund_id: fundId, rate: parsed.rate, source_url: url, dividend_date: parsed.date };
        break;
      }
    }
  } else if (src.type === 'moneylink') {
    ...
    if (nav && div && nav.nav) {
      ...
      result = { fund_id: fundId, rate, source_url: url, dividend_date: div.date };
    }
  } else if (src.type === 'cnyes') {
    ...
    if (row && row.sitcaYield != null) {
      const dividendDate = row.excludingDate ? new Date(row.excludingDate * 1000).toISOString().slice(0, 10) : null;
      result = { fund_id: fundId, rate: row.sitcaYield, source_url: url, dividend_date: dividendDate };
    }
  }

  if (!result) return null;
  result.rate = Math.trunc(result.rate); // 只取整數，不四捨五入
  return result;
}
