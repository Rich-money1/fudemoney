...
你會收到系統已經算好的技術指標（RSI、均線、布林通道、ATR、近期支撐壓力），以及系統依規則算出的多方（做多）跟空方（做空）兩組「進場區間／止損／止盈」參考錨點。

規則（務必遵守）：
- entry_low/entry_high/stop_loss/take_profit_1/take_profit_2（多方）跟 short_entry_low/short_entry_high/short_stop_loss/short_take_profit_1/short_take_profit_2（空方）這幾個錨點若為 null，代表系統判斷該方向目前沒有明確訊號，你就回傳 null，不要自己編造數字。
- 若錨點不是 null，你可以在錨點基礎上做「合理微調」（±3%以內，貼近K線整數關卡或前波高低點皆可），但不能大幅偏離系統給的錨點，止盈止損的數字必須是根據資料算出來的，不能憑感覺捏造。
- 注意：當趨勢是「空頭」但多方錨點仍非null時，代表是RSI超賣的逆勢反彈訊號（接刀），不是順勢拉回買點，風險比多頭拉回買點高很多——這種情況通常應標為「觀望」，只有在RSI極度超賣且價格站在強力支撐才考慮標為「買進區間」，並在rationale說明這是短線逆勢反彈、需嚴設停損。
- 同理，當趨勢是「多頭」但空方錨點仍非null時，代表是RSI超買的逆勢摸頭放空訊號，軋空風險很高——這種情況通常應標為「觀望」，只有在RSI極度超買且價格卡在強力壓力才考慮標為「放空區間」，並在short_rationale說明這是逆勢摸頭、軋空風險高、需嚴設停損。
- 用一句話判斷多方訊號分類（signal）：只能是「買進區間」「觀望」「避免追高」「跌破止損」「盤整」其中一個。
- 用一句話判斷空方訊號分類（short_signal）：只能是「放空區間」「觀望」「避免追空」「軋空止損」「盤整」其中一個（「避免追空」指價格已跌深，此時才追空風險高；「軋空止損」指價格反彈突破空方止損，既有空單應停損回補）。
- rationale 跟 short_rationale 都用繁體中文寫60-100字，白話解釋目前技術面狀況、為什麼給這個訊號，語氣像顧問跟自己筆記，不要用「保證」「必賺」等字眼——止盈止損只是風險管理參考，不是保證。

只能輸出純JSON，不要加任何說明文字或markdown code fence，格式如下：
{"signal": "...", "rationale": "...", "entry_low": number|null, "entry_high": number|null, "stop_loss": number|null, "take_profit_1": number|null, "take_profit_2": number|null, "short_signal": "...", "short_rationale": "...", "short_entry_low": number|null, "short_entry_high": number|null, "short_stop_loss": number|null, "short_take_profit_1": number|null, "short_take_profit_2": number|null}`;
...
系統計算的參考錨點：
【多方】進場區間：${fmt(anchors.entryLow)} ~ ${fmt(anchors.entryHigh)}　止損：${fmt(anchors.stopLoss)}　止盈1：${fmt(anchors.takeProfit1)}　止盈2（壓力位）：${fmt(anchors.takeProfit2)}
【空方】進場區間：${fmt(anchors.shortEntryLow)} ~ ${fmt(anchors.shortEntryHigh)}　止損：${fmt(anchors.shortStopLoss)}　止盈1：${fmt(anchors.shortTakeProfit1)}　止盈2（支撐位）：${fmt(anchors.shortTakeProfit2)}
...
請輸出JSON判讀結果。`;
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 800, // 多方+空方兩組判讀，輸出JSON變大，從512提高到800
...
async function analyzeSymbol(item) {
  ...
  return {
    ...
    short_signal: parsed.short_signal ?? null,
    short_rationale: parsed.short_rationale ?? null,
    short_entry_low: parsed.short_entry_low ?? null,
    short_entry_high: parsed.short_entry_high ?? null,
    short_stop_loss: parsed.short_stop_loss ?? null,
    short_take_profit_1: parsed.short_take_profit_1 ?? null,
    short_take_profit_2: parsed.short_take_profit_2 ?? null,
