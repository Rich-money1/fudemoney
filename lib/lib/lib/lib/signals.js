...
module.exports = { runSignals, analyzeSymbol, buildUniverse };
  ...
  /* 清掉不在目前自選股/持倉裡的舊訊號殘影（例如代碼大小寫不同造成的重複、或已從清單移除的標的） */
  const { data: allSignalRows } = await supabase.from('signals').select('symbol');
  const universeSet = new Set(symbols);
  const staleSymbols = (allSignalRows || []).map(r => r.symbol).filter(s => !universeSet.has(s));
  if (staleSymbols.length > 0) {
    await supabase.from('signals').delete().in('symbol', staleSymbols);
  }
