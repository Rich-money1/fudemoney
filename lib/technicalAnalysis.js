...
const EMPTY_ANCHORS = {
  entryLow: null, entryHigh: null, stopLoss: null, takeProfit1: null, takeProfit2: null,
  shortEntryLow: null, shortEntryHigh: null, shortStopLoss: null, shortTakeProfit1: null, shortTakeProfit2: null,
};

/* 依規則算出多方（做多）與空方（做空）兩組進場區間/止損/止盈的客觀錨點：
   多方：
   ...
   空方（跟多方鏡射）：
   - 空頭趨勢：反彈到「布林中軌(SMA20)～布林上軌」之間找空點
   - 非空頭但RSI>70（超買）：貼近布林上軌找摸頭放空點
   - 其餘（多頭、無明確反彈訊號）：不給進場區間，代表當下觀望
   止損＝進場區間上緣再加 1.5倍ATR，並取近60日壓力價與該值中較保守（低）者，避免止損設在壓力之上太遠
   止盈1＝進場區間下緣扣 2倍ATR，止盈2＝近60日支撐價 */
function computeAnchors(ind) {
  const { sma20, bb, atr14, support, resistance, trend, rsi14 } = ind;
  if (atr14 == null || bb == null) return { ...EMPTY_ANCHORS };
  ...
  let shortEntryLow = null, shortEntryHigh = null;
  if (trend === '空頭') {
    shortEntryLow = sma20;
    shortEntryHigh = Math.max(bb.upper, sma20);
  } else if (rsi14 != null && rsi14 > 70) {
    shortEntryLow = bb.mid;
    shortEntryHigh = bb.upper;
  }

  const round = v => v == null ? null : +v.toFixed(4);
  const result = { ...EMPTY_ANCHORS };

  if (entryLow != null && entryHigh != null) {
    const rawStop = entryLow - atr14 * 1.5;
    result.entryLow = round(entryLow);
    result.entryHigh = round(entryHigh);
    result.stopLoss = round(support != null ? Math.max(rawStop, support * 0.98) : rawStop);
    result.takeProfit1 = round(entryHigh + atr14 * 2);
    result.takeProfit2 = round(resistance);
  }

  if (shortEntryLow != null && shortEntryHigh != null) {
    const rawStop = shortEntryHigh + atr14 * 1.5;
    result.shortEntryLow = round(shortEntryLow);
    result.shortEntryHigh = round(shortEntryHigh);
    result.shortStopLoss = round(resistance != null ? Math.min(rawStop, resistance * 1.02) : rawStop);
    result.shortTakeProfit1 = round(shortEntryLow - atr14 * 2);
    result.shortTakeProfit2 = round(support);
  }
